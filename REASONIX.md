...
- **Phalcon** — C-extension PHP framework; runs as php-fpm
...
- `src/App/` — application code (Controllers, Modules, Console, Projects)
...
- `docker-compose up -d` — start dev environment (nginx:8071, MySQL:13306, Redis:16379)
...
- **Runtime mode detection**: code branches on `IS_PHP_FPM`,  `IS_TASK` constants — logic paths differ significantly between modes.
