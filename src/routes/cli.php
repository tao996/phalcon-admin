...
CliRouter::add('test', function ($params) {
    system(PATH_ROOT . 'vendor/bin/phpunit -c ./phpunit.xml '.join(' ',$params));
}, 'run phpunit test');
...
CliRouter::add('migration', function () {
    if (file_exists(PATH_ROOT . 'phalcon-migrations/index.php')) {
...
CliRouter::add('cc', function ($params) {
    ...
    system(PATH_ROOT . 'vendor/bin/codecept ' . join(' ', $params), $code);
