...
class BaseRbacController extends BaseResponseController
{
     ...
     * @param int $code 错误码 403 没有访问权限
     * @param string $url 跳转地址（一个 URL 地址） 或者 关闭按钮（close）
     * @throws BlankException
     */
    protected function accessDenyResponse(string $msg = '', int $code = 403, string $url = 'close'): void
    {
        if ('' == $msg) {
            $msg = '没有访问的权限';
        }
     ...
     * @var callable | null
     */
    protected $self_rbac_validate = null;
    ...
    protected function rbacInitialize(): void
    {
        ...
        if (is_callable($this->self_rbac_validate)) {
            if (call_user_func($this->self_rbac_validate, $user, $this->action)) {
                return;
            }
        }
