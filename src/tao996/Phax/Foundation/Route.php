...
{
    /**
     * @var array{pattern:string,paths:array{module:string,controller:string,action:string},pathsname:array{module:string,controller:string,action:string},namespace:string,viewpath:string,project:string,isLanguage:bool,language:string,isApi:bool,route:string,pickview:string}
     */
...
//        ddd($this->routerOptions, $this->view()->getViewsDir(), $this->view()->getMainView());
    ...
    {
        return $this->routerOptions['language'] ?: '';
    }

    public function isApiRequest(): bool
    {
        return $this->routerOptions['isApi'] ?: false;
    }
    ...
    {
        // todo 将 controller 和 action 还原
        $classname = $this->routerOptions['namespace'] . '\\' . ucfirst(
                $this->routerOptions['pathsname']['controller']
            ) . 'Controller';
        /**
         * @var $obj Controller
         */
//        ddd($this->routerOptions,$classname);
        $obj = new $classname();
        if (method_exists($obj, 'initialize')) {
            $obj->initialize();
        }
        $action = $this->routerOptions['pathsname']['action'].'Action';
        $data = call_user_func([$obj, $action]);
        if (method_exists($obj, 'jsonResponseData')) {
            return $obj->jsonResponseData($data);
        }
        return $data;
    }
}