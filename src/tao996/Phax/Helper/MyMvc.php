...
class MyMvc
{
    ...
    public function origin(): string
    {
        if (empty($this->origin)) {
            if ($xOrigin = $this->request()->getHeader('x-forwarded-host')) {
                $this->origin = $this->request()->getHeader('x-forwarded-proto') . '://' . $xOrigin;
            } else {
                $this->origin = $this->config()->path('app.url');
                if (empty($this->origin)) {
    ...
    public function url(array $options): string
    {
        ...
        if (MyData::getBool($options, 'language', true)) {
            if (!empty($this->route()->routerOptions['isLanguage'])) {
                $options['language'] = $this->route()->routerOptions['language'];
            } else {
                $options['language'] = '';
            }
        } else {
     ...
     * @param array|bool $mixed 如果为 `true` 则表示 `api` 请求；如果为 `false` 则表示`不需要 origin`；如果为 `array`，则是请求参数
     * @return string
     */
     ...
     * project 请求
     * @param string $path
     * @param array|bool $mixed 如果为 true 则表示 api 请求，如果为 array，则是请求参数
     * @return string
     */
    ...
    public function session(): \Phalcon\Session\Manager
    {
...
//        $lang = $this->di->get('request')->getBestLanguage(); // zh-CN
        ...
        $this->route()->routerOptions['language'] = $this->config()->path('app.locale', 'en');
        return $this->route()->routerOptions['language'];
    }
